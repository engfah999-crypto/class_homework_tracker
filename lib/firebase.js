...
export default function HomeworkTracker() {
  // State หลักที่ดึงมาจาก Firebase
  ...
  useEffect(() => {
    ...
    // 1. ดึงข้อมูลตารางเรียนและการบ้าน (Records)
    ...
    // 2. ดึงข้อมูลประวัติการแก้ไข (Logs) เรียงจากใหม่ไปเก่าจำกัด 50 รายการ
    ...
    // 3. ดึงข้อมูลผู้ใช้งาน (Users)
    const unsubUsers = onSnapshot(collection(db, 'users'), (snapshot) => {
      const usersData = [];
      snapshot.forEach(doc => { usersData.push({ id: doc.id, ...doc.data() }); });
      
      // ถ้าไม่มี User ในระบบเลย ให้สร้าง Admin เริ่มต้นอัตโนมัติ
      if (usersData.length === 0) {
        const initialAdmin = { username: 'Yupparaj', password: 'admin m.2/10', name: 'Yupparaj', role: 'admin', points: 0, badges: [], praises: [], status: 'offline', lastLogin: '-', isLocked: false, forcePwd: false };
        setDoc(doc(db, 'users', 'admin_u1'), initialAdmin);
      } else {
        setUsers(usersData);
        // อัปเดต State ของ CurrentUser หากข้อมูลใน Firebase เปลี่ยน (เช่น โดนล็อกบัญชี)
        if (currentUser) {
  ...
  }, [currentUser]); // เพิ่ม currentUser เป็น dependency เพื่อให้อัปเดตสถานะ
  ...
  const proceedLogin = async (user) => {
    ...
    // อัปเดตเวลาล็อกอินขึ้น Firebase
  ...
  const handleForcePasswordChange = async (e) => {
    ...
    await setDoc(doc(db, 'users', user.id), { password: newPwd, forcePwd: false, status: 'online', lastLogin: now }, { merge: true });
  ...
  const addLog = async (action, detail) => {
    ...
    const newLog = {
      ...
      timestamp: Date.now() // สำหรับ Query เรียงลำดับ
    };
  ...
  const updateRecord = async (dateStr, subjectId, field, value) => {
    ...
    if (currentUser) {
      ...
      if (actionDetail) addLog('แก้ไขข้อมูลรายวิชา', actionDetail);
    }
  };
  ...
  const TimetableView = () => {
    ...
    const handleSave = () => {
      ...
      alert('บันทึกข้อมูลเรียบร้อยแล้ว ข้อมูลถูกอัปโหลดขึ้นเซิร์ฟเวอร์แล้วครับ!');
    };
      ...
      <div className="space-y-6 animate-in fade-in duration-300">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400">ระบบเชื่อมต่อคลาวด์อัตโนมัติ ☁️</p>
          </div>
          <DateNavigator />
        </header>
  ...
  const AdminView = () => {
    ...
    const grantReward = async (userId, type, value, points) => {
      const u = users.find(user => user.id === userId);
      if(!u) return;

      if (type === 'badge' && !u.badges.includes(value)) { 
        await setDoc(doc(db, 'users', userId), { badges: [...u.badges, value], points: u.points + points }, { merge: true });
        addLog('มอบเหรียญรางวัล', `มอบเหรียญ "${value}" ให้กับ ${u.name}`); 
      }
      if (type === 'praise' && !u.praises.includes(value)) { 
        await setDoc(doc(db, 'users', userId), { praises: [...u.praises, value], points: u.points + points }, { merge: true });
        addLog('ชื่นชมผลงาน', `ชื่นชม "${value}" ให้กับ ${u.name}`); 
      }
    ...
    const handleAddUser = async (e) => {
      ...
      const newUser = { username: formData.username, password: formData.password, name: formData.name, role: 'writer', points: 0, badges: [], praises: [], status: 'offline', lastLogin: '-', isLocked: false, forcePwd: false };
      
      await setDoc(doc(db, 'users', newUserId), newUser);
      addLog('เพิ่มผู้ใช้งาน', `เพิ่มบัญชีคนจดการบ้าน: ${formData.name}`); 
      setShowAddForm(false); setFormData({ username: '', password: '', name: '' });
    };

    const handleUpdateUser = async (e) => {
      e.preventDefault();
      await setDoc(doc(db, 'users', editUserId), { name: formData.name, username: formData.username, password: formData.password }, { merge: true });
      addLog('แก้ไขผู้ใช้งาน', `แก้ไขข้อมูลคนจดการบ้าน: ${formData.name}`); 
    ...
    const handleDeleteUser = async (id, name) => {
      if (window.confirm(`คุณแน่ใจหรือไม่ว่าต้องการลบบัญชี "${name}"?`)) { 
        ...
        addLog('ลบผู้ใช้งาน', `ลบบัญชีคนจดการบ้าน: ${name}`); 
      }
    };

    const handleResetLogin = async (id, name) => {
      if (window.confirm(`รีเซ็ตรหัสผ่านของ "${name}" เป็น "password" และบังคับเปลี่ยนรหัสใหม่ในการเข้าสู่ระบบครั้งหน้า?`)) {
        await setDoc(doc(db, 'users', id), { password: 'password', forcePwd: true }, { merge: true });
        addLog('รีเซ็ตรหัสผ่าน', `รีเซ็ตรหัสผ่านบัญชี: ${name}`); 
        alert('รีเซ็ตสำเร็จ รหัสผ่านชั่วคราวคือ: password');
      }
    };

    const handleToggleLock = async (id, name, isLocked) => {
      const action = isLocked ? 'ปลดล็อก' : 'ล็อก';
      if (window.confirm(`ต้องการ${action}บัญชี "${name}" ใช่หรือไม่?`)) { 
        await setDoc(doc(db, 'users', id), { isLocked: !isLocked }, { merge: true });
        addLog(`${action}บัญชี`, `${action}บัญชีคนจดการบ้าน: ${name}`); 
      }
    };
            ...
            <div key={user.id} className={`bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border ${user.isLocked ? 'border-red-300 dark:border-red-800' : 'border-slate-200 dark:border-slate-700'} relative`}>
              {editUserId === user.id ? (
                ...
                <>
                  ...
                  <div className="space-y-4">
                    <div>
                      <p className="text-xs font-bold uppercase opacity-50 mb-2">เหรียญเกียรติยศ ({user.badges?.length || 0})</p>
                    ...
                    <div>
                      <p className="text-xs font-bold uppercase opacity-50 mb-2">คำชม ({user.praises?.length || 0})</p>
      ...
      <div className="space-y-6 animate-in fade-in duration-300 max-w-3xl mx-auto">
        <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-sm border border-slate-100 dark:border-slate-700 text-center relative overflow-hidden">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 relative z-10 mb-8">
            ...
            <div className="bg-purple-50 dark:bg-slate-700 rounded-2xl p-4"><p className="text-sm text-slate-500 font-bold mb-1">เหรียญรางวัล</p><p className="text-3xl font-black text-purple-600 dark:text-purple-400">{currentUser.badges?.length || 0}</p></div>
